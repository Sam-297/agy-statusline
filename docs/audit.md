...
1. **Distribution: GitHub Release tarballs installed with npm, and the same installer works from a git clone.** The user said "idk" (npm registry at first; changed on 2026-09-25 because the user has no npm account). The npm name `agy-statusline` is **taken** (by pkradioman, May 2026), so the package is **`@sam-297/agy-statusline`**, installed with `npm i -g https://github.com/Sam-297/agy-statusline/releases/latest/download/agy-statusline.tgz`, and the command stays `agy-statusline`. `agy-statusline install` registers `'<node>' '<script>'` on Linux/macOS (single-quoted for `sh -c`). On Windows it registers `node <script>` when the path has only plain characters (fastest), else the bare npm shim `agy-statusline` (works even for usernames with spaces). `agy-statusline uninstall` restores the previous `statusLine`. The `agy plugin install` flow, `hooks/`, `hooks.json` and `plugin.json` are dropped.
...
3. **Theme lineup: keep all 6, fixed** (user's choice), plus `powerline` and `quota` added on 2026-09-25. They get shared helpers, real data only, responsive layouts, and no PII by default.
